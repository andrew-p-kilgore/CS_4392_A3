...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Catmull-Rom Curve On/Off", m_curve->curve_enabled);
...
void Renderer::display(GLFWwindow* window)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (m_aircraft_animation->is_moving) {
			m_aircraft_animation->animate();
		}
		else {
			m_aircraft_animation->time_start();
		}
